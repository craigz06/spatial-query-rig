...
Revision Notes (v3.0.0 patch, July 28, 2026):
Bumped CLAUDE_MAX_TOKENS from 1024 to 4096. At 1024, the object-comparison
JSON response was getting cut off mid-value (observed truncation at ~116
chars into the response), causing a JSON parse failure and a dropped
result. Also added a stop_reason print in the parse-failure handler so
any future truncation is immediately diagnosable instead of just showing
a bare parse error.
    ...
    try:
        ...
        if stop_reason is not None:
            print(f"  stop_reason: {stop_reason}"
                  + ("  <-- response was cut off by max_tokens; raise CLAUDE_MAX_TOKENS"
...
def run_camera_pipeline():
    ...
    print("=================================================================")
    print("DCI v3.0.0 — Direct Camera Interface")
